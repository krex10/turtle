...
?> <table >
	...
	while ($row= mysql_fetch_array($result)) {
		...
		<tr>
			...
			<td width="60" style=<?php echo $padding_left; ?> > <p class=<? echo "'$row[rentalId]'"; ?> style="background: url(house.png) no-repeat; padding: 20px 5px 12px 14px;"> <?php echo "{$row['distance']}";  ?></p>
			</td>
...
if (!((($s+$limit)/$limit)==$pages) && $pages!=1) {
	...
	<button class="page_buttons" onclick=<?php $sort_var = "'sort(\"".$supertrimmed."\", \"$sort\", \"$filter\", \"$news\" )'"; echo $sort_var; ?> style="margin-left:115px;" >Next Page</button> <?
}
?>
			<div id="filters">
